...
public partial class WebSite5_production_Flybuy_Verification : System.Web.UI.Page
{
    ...
    public static void insertFlybuy_Verification(string verification)
    {
        ...
        string query = "insert into Flybuy_Verification values('"+ flyVerificationID + "','"+ verification.ToUpper() + "','Active','"+time.ToString(format)+"','')";
        ...
        string details = "Verification created: Name: " + verification.ToUpper();
...

    {
        ...
        string pageName = "Verification";
        string details = "Verification:" + flyVerification.ToUpper() + " deleted";
        int insertlog1 = Queries.admin_Log(pageName, details, user, DateTime.Now.ToString());
    }
    [WebMethod]
    ...
    {
        HttpContext.Current.Session["Verification_Name"] ="";
        HttpContext.Current.Session["Verification_Status"] = "";
        ...
        while (reader.Read())
        {
            HttpContext.Current.Session["Verification_Name"] = reader.GetString(0);
            HttpContext.Current.Session["Verification_Status"] = reader.GetString(1);
        }
        if (HttpContext.Current.Session["Verification_Name"].ToString().Equals(fly_Verification))
        { }
        else
        {
            string pageName = "Verification";
            string details = "Name changed from: " + HttpContext.Current.Session["Verification_Name"].ToString() + "To: " + fly_Verification.ToUpper();
            int insertlog1 = Queries.admin_Log(pageName, details, user, DateTime.Now.ToString());
        }

        if (HttpContext.Current.Session["Verification_Status"].ToString().Equals(status))
        { }
        else
        {
            string pageName = "Verification";
            string details = "status changed from: " + HttpContext.Current.Session["Verification_Status"].ToString() + "To: " + status;
            int insertlog1 = Queries.admin_Log(pageName, details, user, DateTime.Now.ToString());
        }
