...
public partial class WebSite5_production_Flybuy_PreArrival : System.Web.UI.Page
{
    ...
    public static void insertFlybuy_PreArrival(string fly_PreArrival)
    {
        ...
        string query = "insert into Flybuy_Pre_Arrival(Pre_Arrival_ID, Pre_Arrival_Name, Pre_Arrival_Status, Pre_Arrival_Create_Date,Pre_Arrival_Expiry_Date) values('"+flyPreArrivalID+"', '"+fly_PreArrival.ToUpper()+"', 'Active', '"+time.ToString(format)+"','') ";
        ...
        string details = "Pre Arrival created: Name: " + fly_PreArrival.ToUpper();
    ...
    {
       HttpContext.Current.Session["Pre_Arrival_Name"]="";
       HttpContext.Current.Session["Pre_Arrival_Status"] = "";
        string user = HttpContext.Current.Session["username"].ToString();
        string conn = ConfigurationManager.ConnectionStrings["DBConnectionString"].ConnectionString;
        SqlConnection sqlcon1 = new SqlConnection(conn);
        sqlcon1.Open();
        ...
        while (reader.Read())
        {
            HttpContext.Current.Session["Pre_Arrival_Name"] = reader.GetString(0);
            HttpContext.Current.Session["Pre_Arrival_Status"] = reader.GetString(1);
        }
        if (HttpContext.Current.Session["Pre_Arrival_Name"].ToString().Equals(fly_PreArrival))
        { }
        else
        {
            string pageName = "Pre Arrival";
            string details = "Name changed from: " + HttpContext.Current.Session["Pre_Arrival_Name"].ToString() + "To: " + fly_PreArrival.ToUpper();
            int insertlog1 = Queries.admin_Log(pageName, details, user, DateTime.Now.ToString());
        }

        if (HttpContext.Current.Session["Pre_Arrival_Status"].ToString().Equals(status))
        { }
        else
        {
            string pageName = "Pre Arrival";
            string details = "status changed from: " + HttpContext.Current.Session["Pre_Arrival_Status"].ToString() + "To: " + status;
            int insertlog1 = Queries.admin_Log(pageName, details, user, DateTime.Now.ToString());
        }
