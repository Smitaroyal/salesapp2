...
public partial class WebSite5_production_To_Opc : System.Web.UI.Page
{
    ...
    public static void insertToOpc(string toOpcName, string venue,string venueGroup)
    {
        ...
        string details = "TO OPC Created : Name:" + toOpcName.ToUpper() + ", Venue:" + venue;
    ...
    {
       HttpContext.Current.Session["TO_Name"]="";
        string user = HttpContext.Current.Session["username"].ToString();
        string conn = ConfigurationManager.ConnectionStrings["DBConnectionString"].ConnectionString;
        SqlConnection sqlcon1 = new SqlConnection(conn);
        sqlcon1.Open();
        ...
        while (reader.Read())
        {
            HttpContext.Current.Session["TO_Name"] = reader.GetString(0);
        ...
        string pageName = "TO OPC";
        string details = "TO OPC : " + HttpContext.Current.Session["TO_Name"].ToString() + " deleted";
        int insertlog1 = Queries.admin_Log(pageName, details, user, DateTime.Now.ToString());
    }
    ...
    {
        HttpContext.Current.Session["TO_Name1"] = ""; 
        HttpContext.Current.Session["TO_Status1"] = ""; 
        ...
        while (reader.Read())
        {
            HttpContext.Current.Session["TO_Name1"] = reader.GetString(1);
            HttpContext.Current.Session["TO_Status1"] = reader.GetString(2);

            HttpContext.Current.Session["venue1"] = reader.GetString(3);
           
        }

        if (HttpContext.Current.Session["TO_Name1"].ToString().Equals(toOpcName))
        { }
        else
        {
            string pageName = "TO OPC";
            string details = "TO OPC changed from: " + HttpContext.Current.Session["TO_Name1"].ToString() + " To: " + toOpcName.ToUpper();
            int insertlog1 = Queries.admin_Log(pageName, details, user, DateTime.Now.ToString());
        }

        if (HttpContext.Current.Session["TO_Status1"].ToString().Equals(toOpcStatus))
        { }
        else
        {
            string pageName = "TO OPC";
            string details = "Status changed from: " + HttpContext.Current.Session["TO_Status1"].ToString() + " To: " + toOpcStatus;
            int insertlog1 = Queries.admin_Log(pageName, details, user, DateTime.Now.ToString());
        }
