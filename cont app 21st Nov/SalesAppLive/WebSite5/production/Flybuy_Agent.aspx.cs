...
public partial class WebSite5_production_Flybuy_Agent : System.Web.UI.Page
{
    ...
    public static void insertFlybuy_Agent(string flyAgentName)
    {
        ...
        string details = "Flybuy Agent created: Name: " + flyAgentName.ToUpper() ;
    ...
    {
        HttpContext.Current.Session["FAgent_Name"]="";
        ...
        while (reader.Read())
        {
            HttpContext.Current.Session["FAgent_Name"] = reader.GetString(0);
            HttpContext.Current.Session["status1"] = reader.GetString(1);
        }
        if (HttpContext.Current.Session["FAgent_Name"].ToString().Equals(flyAgentName))
        { }
        else
        {
            string pageName = "Flybuy Agent";
            string details = "Name changed from: " + HttpContext.Current.Session["FAgent_Name"].ToString() + "To: " + flyAgentName.ToUpper();
            int insertlog1 = Queries.admin_Log(pageName, details, user, DateTime.Now.ToString());
        }
