...
public partial class WebSite5_production_InhouseStats : System.Web.UI.Page
{
    ...
    [WebMethod]
    public static void insertInhouseStats(string groupvenue, string date,string val,string val1,string val2,string val3)
    {
        string user = HttpContext.Current.Session["username"].ToString();
          HttpContext.Current.Session["log"] = "";
            ...
            {
                string query = "insert into InhouseStats values ('" + groupvenue + "','" + array[i] + "','" + date1 + "','" + array1[i] + "','" + array2[i] + "','" + array3[i] + "','" + returnNumber + "')";
                SqlCommand cmd1 = new SqlCommand(query, sqlcon);
                cmd1.ExecuteNonQuery();

            HttpContext.Current.Session["log"] += "Group venue: "+groupvenue + " , Sub Venue: " + array[i] + " ,Date: " + date1+ " ,Occupied: " + array1[1]  + " ,N Tours " + array2[i] + " ,N Deals " + array3[i] + " ";
    ...
    public static string getAllInhouseStats(string date,string groupVenue)
    {
        ...
        SqlConnection sqlcon = new SqlConnection(conn);
        string JSON = "{\n \"names\":[";
        string query = "select SRN,Group_Venue,Sub_venue,convert(varchar(50),IDate,	105),Occu,NTour,NDeal,Weekno from InhouseStats where IDate='" + date1 + "' and Group_Venue='"+groupVenue+"';";
        sqlcon.Open();
        SqlCommand cmd = new SqlCommand(query, sqlcon);
        SqlDataReader reader = cmd.ExecuteReader();
        if (reader.HasRows)
        {
            while (reader.Read())
            {
                ...
                JSON += "[\"" + ID + "\",\"" + groupvenue + "\",\"" + subVenue + "\",\"" + dates + "\",\"" + occupied + "\",\"" + tours + "\",\"" + deals + "\",\"" + weekno + "\"],";
    ...
    [WebMethod]
    public static void updateInnhouseStats(string valID, string valOccupied, string valNTours, string valNDeals)
    {
        ...
        {
            string query1 = "select Group_Venue, Sub_venue, convert(varchar,IDate) , Occu, NTour, NDeal from InhouseStats where SRN = '" + ID[i] + "'";
            SqlCommand cmd = new SqlCommand(query1, sqlcon1);
            SqlDataReader reader = cmd.ExecuteReader();
        ...
        for (int i = 0; i <= array.Length - 1; i++)
        {
            ...
            string query = "update InhouseStats set Occu='" + array1[i] + "',NTour='" + array2[i] + "',NDeal='" + array3[i] + "' where SRN='" + array[i] + "';";
