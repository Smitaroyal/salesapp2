...
public partial class WebSite5_production_Country : System.Web.UI.Page
{
    ...
    public static void insertContractInitials(string venue,string club,string nationality,string initial,string inc,string type,string venueName)
    {
        ...
        else
        {
            ...
            id = Convert.ToInt32(val) + 1;
            contInitID = value + id;
            string query1 = "insert into Contract_ResortCode_Generation(Contract_Resort_ID,Contract_Resort_Name,Contract_Resort_Status,Contract_Resort_Created_Date,office,Venue,Code,Increment_Value,Nationality) values('" + contInitID + "','" + club + "','Active','" + time.ToString(format) + "','" + office + "','" + venueName + "','" + initial.ToUpper() + "','" + inc + "','" + nationality + "');";
            SqlCommand cmd2 = new SqlCommand(query1, sqlcon);
            cmd2.ExecuteNonQuery();
...

    {
        ...
        if (value == "TimeShare")
        {
            ...
            SqlConnection sqlcon = new SqlConnection(conn);
            string JSON = "{\n \"names\":[";
            string query = "select distinct Contract_Club_Name  from Contract_Club where Office='"+office+ "' and Contract_Club_Status='Active';";
            sqlcon.Open();
            SqlCommand cmd = new SqlCommand(query, sqlcon);
            SqlDataReader reader = cmd.ExecuteReader();
            while (reader.Read())
        ...
        else
        {
            ...
            SqlConnection sqlcon = new SqlConnection(conn);
            string JSON = "{\n \"names\":[";
            string query = "select distinct Contract_Resort_Name from Contract_ResortCode_Generation  where office='"+office+"' and Contract_Resort_Status='Active'";
            sqlcon.Open();
            SqlCommand cmd = new SqlCommand(query, sqlcon);
            SqlDataReader reader = cmd.ExecuteReader();
            while (reader.Read())
...

    {
        ...
        else
        {
            ...
            string query = "select Contract_Resort_ID,Contract_Resort_Name,Contract_Resort_Status,v.Venue_ID,venue,Code,Increment_Value,Nationality from Contract_ResortCode_Generation cg join venue v on  cg.Venue=v.Venue_Name where office = '" + office + "' and cg.Venue like '" + value + "%' and cg.Contract_Resort_Status='Active' UNION  select Contract_Club_ID,Contract_Club_Name,Contract_Club_Status,cc.Venue_ID,v.Venue_Name,Contract_Code,Increment_Value,Nationality  from Contract_Club cc  join venue v on cc.Venue_ID = v.Venue_ID  where Office = '" + office + "' and v.Venue_Name like '" + value + "%' and Contract_Club_Status='Active';";
            sqlcon.Open();
            SqlCommand cmd = new SqlCommand(query, sqlcon);
            SqlDataReader reader = cmd.ExecuteReader();
            while (reader.Read())
...

    {
        ...
        if (type == "TimeShare")
        {
            ...
            string query1 = "select Contract_Club_Name,Contract_Club_Status,Contract_Code,Venue_ID,Increment_Value,Nationality from Contract_Club where Office='"+office+"' and Contract_Club_ID='"+contractID+"';";
        ...
        else
        {
            ...
            string query1 = "select Contract_Resort_Name,Contract_Resort_Status,venue,Code,Increment_Value,Nationality from Contract_ResortCode_Generation where office='"+office+"' and Contract_Resort_ID='"+contractID+"';";
            SqlCommand cmd = new SqlCommand(query1, sqlcon1);
            SqlDataReader reader = cmd.ExecuteReader();
            while (reader.Read())
            {
