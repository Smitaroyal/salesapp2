...
public partial class WebSite5_production_Property_Fee : System.Web.UI.Page
{
    ...
    static string user;
    static float charge_Property_fee_per_year;
    static float charge_member_fee;
    static string charge_year;
    static string charge_status;
    ...
    public static void updatePropertyFee(string chargeID, string propertyPerYear, string memberFee,string ChargeYear, string status)
    {
        ...
        while (reader.Read())
        {
            charge_Property_fee_per_year = reader.GetFloat(0);
            charge_member_fee = reader.GetFloat(1);
            charge_year = reader.GetString(2);
            charge_status = reader.GetString(3);
        }
        if (charge_Property_fee_per_year.Equals(propertyPerYear))
        { }
        else
        {
            string pageName = "Property Fee";
            string details = "charge_Property_fee_per_year changed from: " + charge_Property_fee_per_year + "To: " + propertyPerYear;
            int insertlog1 = Queries.admin_Log(pageName, details, user, DateTime.Now.ToString());
        }

        if (charge_member_fee.Equals(memberFee))
        { }
        else
        {
            string pageName = "Property Fee";
            string details = "charge_member_fee changed from: " + charge_member_fee + "To: " + memberFee;
            int insertlog1 = Queries.admin_Log(pageName, details, user, DateTime.Now.ToString());
        }

        if (charge_year.Equals(ChargeYear))
        { }
        else
        {
            string pageName = "Property Fee";
            string details = "charge_year changed from: " + charge_year + "To: " + ChargeYear;
            int insertlog1 = Queries.admin_Log(pageName, details, user, DateTime.Now.ToString());
        }


        if (charge_status.Equals(status))
        { }
        else
        {
            string pageName = "Property Fee";
            string details = "charge_status changed from: " + charge_status + "To: " + status;
            int insertlog1 = Queries.admin_Log(pageName, details, user, DateTime.Now.ToString());
        }
