    ...
    public static void updateUsers(string username, string password, string name, string email, string office, string group, string title,string val,string vals,string vals1,string rVal,string rVal1,string aVal,string aVal1)
    {
        ...
        for (int i = 0; i <= array.Length - 1; i++)
        {
            ...
            while (reader.Read())
            {
                ...
                string query2 = "insert into user_group_access ([Group Id],Name,Title,PageName,office,ParentNode,AccessName,PageType,Username,page_order,Groups) values('"+group+"','"+pagename+"','"+ title + "','"+pageurl+"','"+office1+"','"+parentNode+"','"+AcessNode+"','"+type+"','"+username+"','"+order+"','"+groups+"')";
