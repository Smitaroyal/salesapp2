...
public partial class WebSite5_production_Property_Fee : System.Web.UI.Page
{
    ...
    [WebMethod]
    public static void insertPropertyFee( string propertyPerYear ,string memberFee,string ChargeYear,string propertyPerYearAlt,string memberFeeAlt)
    {
        ...
        string query = "insert into Property_fee_charge([charge_ID],[charge_No_Of_Points],[charge_Property_fee_per_year],[charge_Property_fee],[charge_member_fee],[charge_total_charge],[charge_year],[charge_status],[charge_Insert_Date],[charge_Expiry_Date],charge_Property_fee_per_year_Alt,charge_member_fee_Alt) values('" + propertyFeeID + "','0','"+ propertyPerYear + "','0','" + memberFee + "','0','" + ChargeYear + "','Active','"+time.ToString(format)+"','','"+propertyPerYearAlt+"','"+memberFeeAlt+"');";
        ...
        string details = "Property Fee created: propertyPerYear: " + propertyPerYear+ ", memberFee: "+ memberFee+ ", ChargeYear: "+ ChargeYear+ ",propertyPerYear Alt:"+propertyPerYearAlt+ ",memberFee Alt :"+memberFeeAlt;
    ...
    public static string getAllPropertyFee()
    {
        ...
        while (reader.Read())
        {
            ...
            double propertyPerYearAlt = reader.GetDouble(10);
            double memberFeeAlt = reader.GetDouble(11);

            JSON += "[\"" + propertyFeeID + "\" ,\"" + propertyPerYear + "\",\"" + memberFee + "\",\"" + ChargeYear + "\",\"" + status + "\",\"" + propertyPerYearAlt + "\",\"" + memberFeeAlt + "\"],";
    ...
    [WebMethod]
    public static void updatePropertyFee(string chargeID, string propertyPerYear, string memberFee,string ChargeYear, string status,string propertyPerYearAlt,string memberFeeAlt)
    {
        ...
        HttpContext.Current.Session["charge_Property_fee_per_year_Alt"] = "";
        HttpContext.Current.Session["charge_member_fee_Alt"] = "";
        string user = HttpContext.Current.Session["username"].ToString();
        string conn = ConfigurationManager.ConnectionStrings["DBConnectionString"].ConnectionString;
        SqlConnection sqlcon1 = new SqlConnection(conn);
        sqlcon1.Open();

        string query1 = "select charge_Property_fee_per_year,charge_member_fee,charge_year,charge_status,charge_Property_fee_per_year_Alt,charge_member_fee_Alt from Property_fee_charge where charge_ID='" + chargeID+"'";
        SqlCommand cmd = new SqlCommand(query1, sqlcon1);
        SqlDataReader reader = cmd.ExecuteReader();
        while (reader.Read())
        {
            HttpContext.Current.Session["charge_Property_fee_per_year"] = reader.GetDouble(0);
            HttpContext.Current.Session["charge_member_fee"] = reader.GetDouble(1);
            HttpContext.Current.Session["charge_year"] = reader.GetString(2);
            HttpContext.Current.Session["charge_status"] = reader.GetString(3);
            HttpContext.Current.Session["charge_Property_fee_per_year_Alt"] = reader.GetDouble(4);
            HttpContext.Current.Session["charge_member_fee_Alt"] = reader.GetDouble(5);
        }
        ...
        if (HttpContext.Current.Session["charge_Property_fee_per_year_Alt"].ToString().Equals(propertyPerYearAlt))
        { }
        else
        {
            string pageName = "Property Fee";
            string details = "charge_Property_fee_per_year_Alt changed from: " + HttpContext.Current.Session["charge_Property_fee_per_year_Alt"].ToString() + "To: " + propertyPerYearAlt;
            int insertlog1 = Queries.admin_Log(pageName, details, user, DateTime.Now.ToString());
        }


        if (HttpContext.Current.Session["charge_member_fee_Alt"].ToString().Equals(memberFeeAlt))
        { }
        else
        {
            string pageName = "Property Fee";
            string details = "charge_member_fee_Alt changed from: " + HttpContext.Current.Session["charge_member_fee_Alt"].ToString() + "To: " + memberFeeAlt;
            int insertlog1 = Queries.admin_Log(pageName, details, user, DateTime.Now.ToString());
        }
        ...
        if (status == "Active")
        {
            ...
            string query = "update Property_fee_charge set [charge_Property_fee_per_year]='"+ propertyPerYear + "',[charge_member_fee]='"+ memberFee + "',[charge_year]='"+ ChargeYear + "',[charge_status]='"+ status + "',charge_Property_fee_per_year_Alt='"+propertyPerYearAlt+ "',charge_member_fee_Alt='"+memberFeeAlt+"' where [charge_ID]='" + chargeID + "';";
            SqlCommand cmd2 = new SqlCommand(query, sqlcon);
            cmd2.ExecuteNonQuery();
        }
        else
        {
            DateTime time = DateTime.Now;
            string format = "yyyy-MM-dd HH:mm:ss:sss";
            string query = "update Property_fee_charge set [charge_Property_fee_per_year]='" + propertyPerYear + "',[charge_member_fee]='" + memberFee + "',[charge_year]='" + ChargeYear + "',[charge_status]='" + status + "',[charge_Expiry_Date]='"+time.ToString(format)+ "',charge_Property_fee_per_year_Alt='" + propertyPerYearAlt + "',charge_member_fee_Alt='" + memberFeeAlt + "' where [charge_ID]='" + chargeID + "';";
            SqlCommand cmd2 = new SqlCommand(query, sqlcon);
            cmd2.ExecuteNonQuery();
        }
