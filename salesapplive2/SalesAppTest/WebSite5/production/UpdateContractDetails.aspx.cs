...
public partial class WebSite5_production_UpdateContractDetails : System.Web.UI.Page
{
    ...
    [WebMethod]
    public static string getMarketingProgramOnContractNo(string contractNo)
    {
        string profileID = "";
        string groupVenue = "";
        string venue = "";
        ...
        SqlConnection sqlcon = new SqlConnection(conn);
        String JSON = "{\n \"names\":[";
        string query = "select distinct p.Profile_ID,p.Profile_Group_Venue,p.Profile_Venue from ContractDetails_Indian cd join profile p on cd.ProfileID=p.Profile_ID where ContractNo='" + contractNo + "'";
        sqlcon.Open();
        SqlCommand cmd = new SqlCommand(query, sqlcon);
        SqlDataReader reader = cmd.ExecuteReader();
        if (reader.HasRows)
        {
            ...
            SqlConnection sqlcon1 = new SqlConnection(conn);
            string query1 = "select distinct Marketing_Program_abbrv,Marketing_Program_Name,'1' orders from Marketing_Program where Marketing_Program_abbrv in (select distinct Profile_Marketing_Program from profile where Profile_ID = '"+profileID+"') union select distinct mp.Marketing_Program_abbrv,mp.Marketing_Program_Name,'' orders from Marketing_Program mp join Venue_Group vg on vg.Venue_Group_ID = mp.Venue_Group_ID join venue v on v.Venue_ID = vg.Venue_ID where mp.Marketing_Program_abbrv not in (select Profile_Marketing_Program from profile p where Profile_ID = '"+profileID+"') and vg.Venue_Group_Name = '"+groupVenue+"' and v.Venue_Name = '"+venue+"' and mp.Marketing_Program_Status = 'Active' order by orders desc";
            sqlcon1.Open();
            SqlCommand cmd1 = new SqlCommand(query1, sqlcon1);
            SqlDataReader reader1 = cmd1.ExecuteReader();
            if (reader1.HasRows)
            {
                while (reader1.Read())
                {
                    string marketingCode = reader1.GetString(0);
                    string marketingPro = reader1.GetString(1);
                    JSON += "[\"" + marketingCode + "\",\"" + marketingPro + "\"],";
                }
                JSON = JSON.Substring(0, JSON.Length - 1);
                JSON += "] \n}";
    ...
    public static void updatedetails(string group, string contractID, string profileID,string contractNo, string dealStatus,string marketingSource,string referralBy,string Comment,string contractType,string oldVolume,string oldAdminFee,string oldTotalTax,string oldDeposit,string oldContractNo,string subVenue,string leadOffice,string dealDate,string marketingProgram)
    {
        ...
        if (group=="Update Deal Status")
        {
            DateTime dtt = DateTime.ParseExact(dealDate, "dd-MM-yyyy", CultureInfo.InvariantCulture);
            string date = dtt.ToString("yyyy-MM-dd");
            HttpContext.Current.Session["dealStatus"] = "";
            HttpContext.Current.Session["DealRegistered_Date"] = "";
            SqlConnection sqlcon1 = new SqlConnection(conn);
            string querylog = "select DealStatus,convert(varchar,DealRegistered_Date)from ContractDetails_Indian where ContractDetails_ID='" + contractID+"'";
            sqlcon1.Open();
            ...
            if (reader.HasRows)
            {
                while (reader.Read())
                {
                    ...
                    HttpContext.Current.Session["DealRegistered_Date"] = reader.GetString(1);
                }
                ...
                if (HttpContext.Current.Session["DealRegistered_Date"].ToString().Equals(date))
                { }
                else
                {
                    string pageName = "Update Contract Details";
                    string details = "Deal Date changed from: " + HttpContext.Current.Session["DealRegistered_Date"].ToString() + "To: " + date;
                    int insertlog1 = Queries.InsertContractLogs_India(profileID, contractID, pageName + ": " + details, user, DateTime.Now.ToString());
                }
            }
            else
            ...
            SqlConnection sqlcon = new SqlConnection(conn);
            string query = "update ContractDetails_Indian set DealStatus='"+ dealStatus + "',DealRegistered_Date='"+ date + "' where ContractDetails_ID='" + contractID+"' and ContractNo='"+ contractNo + "'";
            sqlcon.Open();
        ...
        else if (group == "Update Marketing Source SPI")
        {
            ...
            HttpContext.Current.Session["Profile_Marketing_Program"] = "";
            SqlConnection sqlcon2 = new SqlConnection(conn);
            string querylog = "select Sale_Marketing_Source,SubVenue,leadoffice,Profile_Marketing_Program from profile where Profile_ID = '" + profileID+"'";
            sqlcon2.Open();
            ...
            if (readerlog.HasRows)
            {
                while (readerlog.Read())
                {
                    ...
                    HttpContext.Current.Session["Profile_Marketing_Program"] = readerlog.GetString(3);
                }
                ...
                if (HttpContext.Current.Session["Profile_Marketing_Program"].ToString().Equals(marketingProgram))
                { }
                else
                {
                    string pageName = "Update Contract Details";
                    string details = "Profile Marketing Program changed from: " + HttpContext.Current.Session["Profile_Marketing_Program"].ToString() + "To: " + marketingProgram;
                    int insertlog1 = Queries.InsertContractLogs_India(profileID, contractID, pageName + ": " + details, user, DateTime.Now.ToString());
                }
            ...
            SqlConnection sqlcon = new SqlConnection(conn);
            string query = "update profile set Sale_Marketing_Source='"+ marketingSource + "',SubVenue='"+subVenue+"',leadOffice='"+leadOffice+ "',Profile_Marketing_Program='"+marketingProgram+"' where Profile_ID='" + profileID+"'";
            sqlcon.Open();
            SqlCommand cmd2 = new SqlCommand(query, sqlcon);
            cmd2.ExecuteNonQuery();
            sqlcon.Close();
